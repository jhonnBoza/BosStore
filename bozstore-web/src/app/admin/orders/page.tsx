  ...
  return (
    <div className="px-6 py-8 sm:px-8">
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          <div key={label} className="border border-zinc-200 dark:border-white/5 bg-zinc-100 dark:bg-zinc-900/50 p-5">
            ...
            <p className="mt-4 font-podium text-4xl text-zinc-300 dark:text-white/20">{value}</p>
            <p className="mt-2 font-inter text-[10px] text-zinc-300 dark:text-white/15">Próximamente</p>
          </div>
        ))}
      </div>
      ...
      <div className="border border-zinc-200 dark:border-white/5">
        {/* Table header */}
        <div className="hidden grid-cols-[2fr_1fr_1fr_1fr_auto] gap-4 border-b border-zinc-200 dark:border-white/10 px-5 py-3 sm:grid">
          {['Cliente', 'Juegos', 'Total', 'Estado', ''].map((h) => (
            <span key={h} className="font-inter text-[9px] uppercase tracking-widest text-zinc-300 dark:text-white/20">
              {h}
            </span>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-28 text-center">
          ...
          <Link
            href="/admin"
            className="mt-6 inline-flex items-center gap-2 border border-zinc-200 dark:border-white/10 px-5 py-2.5 font-inter text-[10px] uppercase tracking-widest text-zinc-400 dark:text-white/30 transition-colors hover:border-zinc-300 dark:border-white/20 hover:text-zinc-500 dark:text-white/50"
          >
